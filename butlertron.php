...
if (empty($options['r'])) {
  ...
  $remote_tracking_branch_command = '';
} else {
  $remote_repository = $options['r'];
  $remote_repository = escapeshellarg($options['r'] . '/');
}
$destination_branch = escapeshellarg($options['d']);
$branches = explode(",", $options['b']);
foreach ($branches AS $branch) {
  $clean_branch = escapeshellarg($branch);
  $git_merge_command = 'git checkout ' . $clean_branch . ' && git merge --no-edit ' . $remote_repository . $destination_branch .
                       ' && git checkout ' . $remote_repository . $destination_branch . ' && git merge --no-edit ' . $clean_branch;
